...
- [x] تحسين: protocol_parser بيرمي ValueError لو مفيش أي قيمة أساسية (WBC/RBC/HGB) - وده بيخلي tcp_server يرد AE تلقائيًا بدل AA (مُجرّب ✓ الحالتين)
- [x] ui/app_window.py - واجهة tkinter بتعرض آخر 50 نتيجة، تحديث تلقائي كل 5 ثواني + زرار Refresh (منطق قراءة البيانات مُجرّب ✓ ضد database.py الحقيقي - الواجهة نفسها محتاجة تجربة بصرية على جهاز فيه شاشة)
- [ ] تأكيد الـ LOINC codes الحقيقية (محتاج اتصال فعلي بالجهاز - مفيش بديل)
- [ ] (اختياري) تشغيل السيرفر والواجهة مع بعض من نفس main.py بـ threading
